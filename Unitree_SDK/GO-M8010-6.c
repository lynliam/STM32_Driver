...
HAL_StatusTypeDef SERVO_Send_recv(MOTOR_send *pData, MOTOR_recv *rData)
{
    ...
    SET_485_DE_UP();
    SET_485_RE_UP();
    HAL_UART_Transmit(&UART_UNITREE_HANDLER, (uint8_t *)pData, sizeof(pData->motor_send_data), 10);


    SET_485_RE_DOWN();
    SET_485_DE_DOWN();

    HAL_UARTEx_ReceiveToIdle_DMA(&UART_UNITREE_HANDLER, (uint8_t *)rData, sizeof(rData->motor_recv_data));
